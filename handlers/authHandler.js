...
const generateToken = (user, res) => {
    ...
    return res
        ...
        .cookie('token', token, {
            httpOnly: true,
            expires: new Date(Date.now() + process.env.JWT_EXPIRY_INT * 24 * 60 * 60 * 1000)
        })
        .json({
            status: 'Success',
            token,
...
exports.sendSignUpEmail = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        if (e.name === 'MongoError' && e.code === 11000)
            return next(new AppError(`${email} already exists. Please try a new one!`, 404));
        throw e;
    }
...
exports.sendLoginEmail = catchAsync(async (req, res, next) => {
    ...
    await new sendEmail(user, `${linkToRedirect}/${generatedToken}`).sendLoginEmail();
    res.status(200).json({
        status: 'Success',
        data: 'A Confirmation Email has been sent to your email. Please Check your inbox'
    });
...
const acceptUserAuthTokens = async ({ userToken }) => {
    ...
    if (!userToken) throw new AppError('You have not provided a required auth token!', 404);

    const authToken = crypto.createHash('sha256').update(userToken).digest('hex');
    const user = await UserModel.findOne({ authToken }).select('+authTokenTimeOut');

    if (!user) throw new AppError('No User with this auth token', 404);
    if (Date.now() > user.authTokenTimeOut)
        throw new AppError('This token has expired. It is valid for only 10 minutes', 404);
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
    ...
    // TODO: correct the link
    await new sendEmail(user, `/reset-password/${token}`).sendPasswordReset(token);
